...
//Avoir l'idcin par le cin number
    public function getIdCinByCinNumber($cinNumber)
    {
        try {
            $result = DB::table('cin')
                ->where('cin_number', $cinNumber)
                ->select('id_cin')
                ->first();
    
            if ($result) {
                return $result->id_cin;
            } else {
                return null; // Cin number not found
            }
        } catch (\Exception $e) {
            throw new \Exception("Erreur lors de la récupération de l'id_cin : " . $e->getMessage());
        }
    }
